...
class LogisticRegressor:
    ...
    def grad_loss(self, *args):
        ...
        grad = 1/m * np.sum((utils.sigmoid(np.dot(X,theta))-y)[:,None]*X, axis=0)
    ...
    def predict(self, X):
        ...
        y_pred = utils.sigmoid(np.dot(X,self.theta)) > 0.5
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        J = 1 / m * np.sum(-y*np.log(h_x) - (1-y)*np.log(1-h_x)) + reg/(2*m) * np.dot(theta[1:], theta.T[1:])
    ...
    def grad_loss(self, *args):
        ...
        grad = 1/m * np.sum((utils.sigmoid(np.dot(X,theta))-y)[:,None]*X, axis=0) + reg/m * (np.hstack([0, theta[1:]]))
